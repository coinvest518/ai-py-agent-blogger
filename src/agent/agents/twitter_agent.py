...
def run(state: dict) -> dict:
    if os.getenv("SKIP_TWITTER", "").lower() in ("1", "true", "yes"):
        ...
        return {"twitter_url": "", "twitter_post_id": "skipped"}

    text = (state.get("tweet_text") or "").strip()
    if not text:
        return {"twitter_url": "", "twitter_post_id": "skipped_no_text"}
    ...
    if zernio_mcp.is_configured():
        ...
        if res.get("success"):
            ...
            return {"twitter_url": "", "twitter_post_id": f"zernio:{post_id}"}
        logger.warning("Zernio X post failed: %s", res.get("error"))

    return {"twitter_url": "", "twitter_post_id": "failed"}
